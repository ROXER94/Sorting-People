...
def human_selection_sort(my_list):
    ''' to take a list and return a sorted version'''
    if len(my_list) == 0 : 
        return my_list
    n = len(my_list)
    for i in range(n-1) :
        ...
        minloc = i
        for j in range(i+1,n) :
            if my_list[j].lessThan(minnie) : # << note change to handle people
                minnie = my_list[j]
                minloc = j
        my_list[minloc] = my_list[i]
        my_list[i] = minnie
...
def radix_sort(my_list):
    ''' assuming my_list to be a list of integers, you might think of proceeding base 10 for clarity
    ...
    print("radix = "+ str(radix) + " to express all numbers in no more than " +str(radix)+ " digits.")
    ...
    for digital in digitised :
        my_list.append(reform_number(digital, base) + small) # so 'unshifting' the values back
    return my_list
...
class Queuer(object) :
    '''a pointer based implementation of a queue -- used in radix sort'''
    def __init__(self, name):
        self.front = None
        self.back = None
        self.length = 0
        self.name = name # when having multiple queues it's helpful to be able to identify each of them
...
tutti = [ ]
for i in range(20):
    tutti.append(Person("Anon",'e', 31))
    tutti[i].randomise_name()
    tutti[i].randomise_height()
''' who is there'''
temp = ''
for folk in tutti :
    temp += str(folk) + "(" + str(folk.my_height) + "), "
print(temp)
'''sort them all'''
arranged = human_selection_sort(tutti)
'''let's see them now sorted by height'''
temp = ''
for folk in arranged :
    temp += str(folk) + "(" + str(folk.my_height) + "), "
print(temp)